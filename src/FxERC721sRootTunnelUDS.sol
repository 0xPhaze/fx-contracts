...
bytes32 constant REGISTER_ERC721_IDS_SIG = keccak256("registerERC721IdsWithChild(address,address,uint256[])");
bytes32 constant DEREGISTER_ERC721_IDS_SIG = keccak256("deregisterERC721IdsWithChild(address,uint256[])");
...
abstract contract FxERC721sRootTunnelUDS is FxBaseRootTunnelUDS {
    ...
    function _registerERC721IdsWithChild(
        ...
        _sendMessageToChild(abi.encode(REGISTER_ERC721_IDS_SIG, abi.encode(collection, to, ids)));
    }

    function _deregisterERC721IdsWithChild(address collection, uint256[] calldata ids) internal virtual {
        _sendMessageToChild(abi.encode(DEREGISTER_ERC721_IDS_SIG, abi.encode(collection, ids)));
    }
}
